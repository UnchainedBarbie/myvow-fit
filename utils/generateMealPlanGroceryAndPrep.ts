/**
 * Build grocery_list (JSON) and prep_guide (text) from PlannedMeals + FoodItems
 * (or MealPlanItems) and persist on MealPlans. Used by MealPlanList and MealPlanDetail.
 *
 * Food categorization: contains-match keyword tables (longest phrases first). Pantry is checked
 * before dairy/meat & fish/produce so oils, condiments, and plant milks map to Pantry.
 */

import { ALTERNATING_A_DAYS, ALTERNATING_B_DAYS } from './initMealPlansDb';
import { foodItemServingToTrackerFields } from './foodItemServingToTrackerRow';

/** Per meal-slot amount: max(DB quantity, parsed leading amount from serving_size), fallback 1. */
function effectiveGroceryPerMealQuantity(
  serving_size: string | null | undefined,
  dbQuantity: number | null | undefined,
): number {
  const { quantity: parsedRaw } = foodItemServingToTrackerFields(serving_size);
  const pq = Number(parsedRaw);
  const parsedOk = Number.isFinite(pq) && pq > 0;

  const dbQ = dbQuantity != null ? Number(dbQuantity) : NaN;
  const dbOk = Number.isFinite(dbQ) && dbQ > 0;

  if (!dbOk) return parsedOk ? pq : 1;
  if (!parsedOk) return dbQ;
  return Math.max(dbQ, pq);
}

/** Same expansion as MealPlanDetail / DayActivePlan: schedule rows → mon–sun keys. */
function expandMealPlanScheduleToWeekdayKeys(rows: { day_of_week: string }[]): Set<string> {
  const out = new Set<string>();
  for (const r of rows) {
    const v = r.day_of_week.toLowerCase().trim();
    if (v === 'alternating_a') {
      ALTERNATING_A_DAYS.forEach((d) => out.add(d));
    } else if (v === 'alternating_b') {
      ALTERNATING_B_DAYS.forEach((d) => out.add(d));
    } else {
      out.add(v);
    }
  }
  return out;
}

/**
 * Days per week this plan’s template applies (each meal slot repeats once per such day).
 * Empty MealPlanSchedule → 7 (same as DayActivePlan: all week).
 */
export async function countWeeklyPlanDaysForGrocery(db: any, planId: number): Promise<number> {
  const scheduleRows = (await db
    .getAllAsync<{ day_of_week: string }>(
      'SELECT day_of_week FROM MealPlanSchedule WHERE meal_plan_id = ?',
      [planId],
    )
    .catch(() => [])) as { day_of_week: string }[];
  const expanded = expandMealPlanScheduleToWeekdayKeys(scheduleRows);
  if (expanded.size === 0) return 7;
  return expanded.size;
}

/**
 * Meal plan rows: `food_name`, optional `serving_size` (unit text only; leading numbers are stripped for display).
 * Tracker / LoggedFoods rows: include `quantity` — summed as-is per row (not parsed from `serving_size`).
 */
export type GroceryPlanFoodRow = {
  food_name: string;
  serving_size?: string | null;
  quantity?: number | null;
};

/** Per-row quantity for aggregation (from fetch: parsed serving + optional DB column). */
function rowGroceryQuantity(r: GroceryPlanFoodRow): number {
  if (r.quantity == null) return 1;
  const q = Number(r.quantity);
  if (!Number.isFinite(q) || q < 0) return 1;
  return q;
}

/**
 * Scale each template row's quantity by scheduled days per week (per-meal qty × days), instead of
 * duplicating rows — same total when aggregating, and matches "2 each of 5 days → 10".
 */
export function multiplyPlanFoodRowsForWeek(
  rows: GroceryPlanFoodRow[],
  factor: number,
): GroceryPlanFoodRow[] {
  const n = Math.max(1, Math.floor(factor));
  if (n <= 1) return rows;
  return rows.map((r) => ({
    food_name: r.food_name,
    serving_size: r.serving_size ?? null,
    quantity: rowGroceryQuantity(r) * n,
  }));
}

/** Internal prep buckets; `grain` historically = pantry / grains / dry goods. */
type FoodKind = 'grain' | 'dairy' | 'meatFish' | 'produce' | 'other';

function sortKeywordsLongestFirst(keywords: readonly string[]): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const k of keywords) {
    const t = k.toLowerCase().trim();
    if (t && !seen.has(t)) {
      seen.add(t);
      out.push(t);
    }
  }
  return out.sort((a, b) => b.length - a.length);
}

/**
 * Multi-word phrases: substring match. Single words: delimiter-aware match so we don't hit
 * substrings like pea→pear, tuna→fortuna, oil→boil, beef→beefsteak.
 */
function matchesKeyword(haystackLower: string, keyword: string): boolean {
  const kw = keyword.toLowerCase();
  if (kw.includes(' ')) {
    return haystackLower.includes(kw);
  }
  const escaped = kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(`(^|[^a-z0-9])${escaped}([^a-z0-9]|$)`, 'i');
  return re.test(haystackLower);
}

/**
 * Pantry: oils, vinegars, condiments, sauces, spices, nuts, seeds, nut butters, canned/dried shelf
 * staples, grains, pasta, bread, tortillas, crackers, flour, sugar, sweeteners, coffee, tea, baking.
 */
const PANTRY_KEYWORDS = sortKeywordsLongestFirst([
  'apple cider vinegar',
  'white wine vinegar',
  'red wine vinegar',
  'rice vinegar',
  'balsamic vinegar',
  'distilled vinegar',
  'worcestershire sauce',
  'worcestershire',
  'vegetable oil',
  'sunflower oil',
  'grapeseed oil',
  'coconut oil',
  'sesame oil',
  'avocado oil',
  'canola oil',
  'olive oil',
  'peanut oil',
  'cooking spray',
  'nonstick spray',
  'almond butter',
  'cashew butter',
  'sunflower butter',
  'walnut butter',
  'peanut butter',
  'nut butter',
  'coconut milk',
  'almond milk',
  'oat milk',
  'soy milk',
  'cashew milk',
  'rice milk',
  'hemp milk',
  'macadamia milk',
  'coconut cream',
  'tomato paste',
  'tomato sauce',
  'pasta sauce',
  'marinara sauce',
  'alfredo sauce',
  'enchilada sauce',
  'bbq sauce',
  'barbecue sauce',
  'teriyaki sauce',
  'oyster sauce',
  'hoisin sauce',
  'fish sauce',
  'soy sauce',
  'tamari sauce',
  'hot sauce',
  'sriracha',
  'chili sauce',
  'buffalo sauce',
  'salad dressing',
  'italian dressing',
  'ranch dressing',
  'caesar dressing',
  'mayonnaise',
  'mirin',
  'rice wine',
  'dijon mustard',
  'yellow mustard',
  'whole grain mustard',
  'stone ground mustard',
  'relish',
  'pickle relish',
  'ketchup',
  'mustard',
  'mayo',
  'vinegar',
  'salsa',
  'guacamole',
  'hummus',
  'tahini',
  'pesto',
  'harissa',
  'gochujang',
  'miso paste',
  'miso',
  'curry paste',
  'curry powder',
  'chili powder',
  'cayenne pepper',
  'garlic powder',
  'onion powder',
  'smoked paprika',
  'chili flakes',
  'red pepper flakes',
  'italian seasoning',
  'poultry seasoning',
  'pumpkin pie spice',
  'garam masala',
  'five spice',
  'bay leaves',
  'bay leaf',
  'vanilla extract',
  'almond extract',
  'baking powder',
  'baking soda',
  'corn starch',
  'cornstarch',
  'arrowroot',
  'xanthan gum',
  'active dry yeast',
  'instant yeast',
  'nutritional yeast',
  'cocoa powder',
  'chocolate chips',
  'chocolate chunk',
  'semi-sweet chocolate',
  'dark chocolate',
  'maple syrup',
  'agave nectar',
  'simple syrup',
  'simple sugar',
  'brown sugar',
  'powdered sugar',
  'confectioners sugar',
  'coconut sugar',
  'date syrup',
  'molasses',
  'corn syrup',
  'stevia',
  'sweetener',
  'granola',
  'cereal',
  'instant oatmeal',
  'rolled oats',
  'steel cut oats',
  'quick oats',
  'oat bran',
  'wheat bran',
  'bread crumbs',
  'panko',
  'cracker',
  'crackers',
  'pretzel',
  'tortilla chip',
  'potato chip',
  'corn chip',
  'tortilla',
  'tortillas',
  'corn tortilla',
  'flour tortilla',
  'wrap',
  'wraps',
  'pita',
  'pita bread',
  'naan',
  'flatbread',
  'bagel',
  'bagels',
  'english muffin',
  'croissant',
  'brioche',
  'sourdough',
  'bread',
  'bun',
  'buns',
  'roll',
  'dinner roll',
  'hoagie roll',
  'slider bun',
  'rice noodle',
  'egg noodle',
  'ramen noodle',
  'udon',
  'soba',
  'vermicelli',
  'spaghetti',
  'penne',
  'fusilli',
  'macaroni',
  'fettuccine',
  'linguine',
  'orzo',
  'lasagna noodle',
  'pasta',
  'noodle',
  'noodles',
  'couscous',
  'polenta',
  'grits',
  'cornmeal',
  'masa harina',
  'tortilla flour',
  'all-purpose flour',
  'bread flour',
  'cake flour',
  'whole wheat flour',
  'almond flour',
  'coconut flour',
  'rice flour',
  'tapioca flour',
  'chickpea flour',
  'flour',
  'rice',
  'brown rice',
  'white rice',
  'jasmine rice',
  'basmati rice',
  'wild rice',
  'arborio rice',
  'sushi rice',
  'quinoa',
  'farro',
  'barley',
  'bulgur',
  'freekeh',
  'millet',
  'buckwheat',
  'amaranth',
  'sorghum',
  'chia seed',
  'chia seeds',
  'flax seed',
  'flaxseed',
  'hemp seed',
  'hemp hearts',
  'pumpkin seed',
  'pumpkin seeds',
  'sunflower seed',
  'sunflower seeds',
  'sesame seed',
  'sesame seeds',
  'poppy seed',
  'mixed nuts',
  'trail mix',
  'walnut',
  'walnuts',
  'pecan',
  'pecans',
  'almond',
  'almonds',
  'cashew',
  'cashews',
  'pistachio',
  'pistachios',
  'hazelnut',
  'hazelnuts',
  'macadamia',
  'brazil nut',
  'pine nut',
  'pine nuts',
  'peanut',
  'peanuts',
  'raisin',
  'raisins',
  'dried cranberry',
  'dried apricot',
  'dried mango',
  'dried fig',
  'prune',
  'prunes',
  'dried dates',
  'medjool dates',
  'deglet noor',
  'beef jerky',
  'turkey jerky',
  'protein bar',
  'casein protein',
  'whey protein',
  'hemp protein',
  'pea protein',
  'collagen powder',
  'protein powder',
  'granola bar',
  'energy bar',
  'canned tomato',
  'canned corn',
  'canned coconut',
  'canned pumpkin',
  'canned soup',
  'broth',
  'stock',
  'chicken broth',
  'beef broth',
  'vegetable broth',
  'bouillon',
  'bone broth',
  'coconut water',
  'sparkling water',
  'club soda',
  'tonic water',
  'coffee',
  'espresso',
  'instant coffee',
  'cold brew',
  'herbal tea',
  'green tea',
  'black tea',
  'chai tea',
  'tea bag',
  'chai',
  'matcha',
  'rooibos',
  'paprika',
  'cumin',
  'coriander',
  'turmeric',
  'cinnamon',
  'nutmeg',
  'cloves',
  'allspice',
  'cardamom',
  'fennel seed',
  'caraway',
  'dill seed',
  'mustard seed',
  'celery seed',
  'star anise',
  'saffron',
  'herbes de provence',
  'everything bagel seasoning',
  'seasoning blend',
  'seasoning mix',
  'taco seasoning',
  'ranch seasoning',
  'italian herbs',
  'dried oregano',
  'dried basil',
  'dried thyme',
  'dried rosemary',
  'dried parsley',
  'dried dill',
  'dried mint',
  'garlic salt',
  'onion salt',
  'celery salt',
  'seasoned salt',
  'kosher salt',
  'sea salt',
  'table salt',
  'black pepper',
  'white pepper',
  'pink salt',
  'msg',
  'honey',
  'syrup',
  'sugar',
  'spice',
  'spices',
  'jam',
  'jelly',
  'preserves',
  'marmalade',
  'apple butter',
  'coconut flakes',
  'shredded coconut',
  'breadcrumbs',
]);

/** Cow / traditional dairy (plant milks hit PANTRY first). */
const DAIRY_KEYWORDS = sortKeywordsLongestFirst([
  'cottage cheese',
  'cream cheese',
  'sour cream',
  'heavy cream',
  'whipping cream',
  'double cream',
  'clotted cream',
  'half and half',
  'half-and-half',
  'coffee creamer',
  'whipped cream',
  'greek yogurt',
  'greek yoghurt',
  'skim milk',
  'whole milk',
  '2% milk',
  '1% milk',
  'buttermilk',
  'chocolate milk',
  'goat milk',
  'sheep milk',
  'raw milk',
  'lactose free milk',
  'mozzarella',
  'parmesan',
  'pecorino',
  'romano',
  'cheddar',
  'swiss cheese',
  'provolone',
  'gouda',
  'brie',
  'camembert',
  'blue cheese',
  'gorgonzola',
  'feta cheese',
  'goat cheese',
  'ricotta',
  'mascarpone',
  'burrata',
  'halloumi',
  'paneer',
  'queso fresco',
  'american cheese',
  'string cheese',
  'cheese slice',
  'shredded cheese',
  'grated cheese',
  'yogurt',
  'yoghurt',
  'kefir',
  'drinkable yogurt',
  'frozen yogurt',
  'ice cream',
  'gelato',
  'sorbet',
  'butter',
  'ghee',
  'clarified butter',
  'margarine',
  'milk',
  'cream',
  'cheese',
]);

const PROTEIN_KEYWORDS = sortKeywordsLongestFirst([
  'boneless skinless chicken',
  'chicken breast',
  'chicken thigh',
  'chicken wing',
  'chicken drumstick',
  'chicken tender',
  'ground chicken',
  'rotisserie chicken',
  'turkey breast',
  'ground turkey',
  'turkey bacon',
  'turkey sausage',
  'duck breast',
  'ground beef',
  'beef steak',
  'ribeye',
  'sirloin',
  'filet mignon',
  'flank steak',
  'skirt steak',
  'brisket',
  'beef roast',
  'stew beef',
  'corned beef',
  'pastrami',
  'roast beef',
  'ground pork',
  'pork chop',
  'pork tenderloin',
  'pork shoulder',
  'pulled pork',
  'pork belly',
  'bacon',
  'pancetta',
  'prosciutto',
  'ham',
  'canadian bacon',
  'sausage',
  'chorizo',
  'andouille',
  'kielbasa',
  'bratwurst',
  'italian sausage',
  'breakfast sausage',
  'hot dog',
  'frankfurter',
  'salami',
  'pepperoni',
  'mortadella',
  'capicola',
  'deli meat',
  'lunch meat',
  'cold cuts',
  'lamb chop',
  'lamb shank',
  'ground lamb',
  'veal cutlet',
  'rabbit',
  'venison',
  'bison',
  'elk',
  'salmon fillet',
  'smoked salmon',
  'lox',
  'tuna steak',
  'canned tuna',
  'tuna pouch',
  'cod fillet',
  'halibut',
  'mahi mahi',
  'snapper',
  'grouper',
  'sea bass',
  'trout',
  'sardine',
  'anchovy',
  'mackerel',
  'herring',
  'fish fillet',
  'fish cake',
  'imitation crab',
  'surimi',
  'shrimp',
  'prawn',
  'scallop',
  'mussel',
  'clam',
  'oyster',
  'lobster',
  'crab meat',
  'crab leg',
  'calamari',
  'squid',
  'octopus',
  'seafood',
  'egg white',
  'egg yolk',
  'egg substitute',
  'liquid eggs',
  'hard boiled egg',
  'tofu',
  'silken tofu',
  'firm tofu',
  'tempeh',
  'seitan',
  'textured vegetable protein',
  'tvp',
  'edamame',
  'black bean',
  'black beans',
  'kidney bean',
  'kidney beans',
  'pinto bean',
  'pinto beans',
  'navy bean',
  'navy beans',
  'cannellini bean',
  'cannellini beans',
  'great northern bean',
  'lima bean',
  'lima beans',
  'fava bean',
  'fava beans',
  'garbanzo',
  'chickpea',
  'chickpeas',
  'refried beans',
  'baked beans',
  'lentil',
  'lentils',
  'split pea',
  'split peas',
  'black-eyed pea',
  'field pea',
  'egg',
  'eggs',
  'chicken',
  'turkey',
  'beef',
  'pork',
  'lamb',
  'veal',
  'fish',
  'tuna',
  'salmon',
  'cod',
  'tilapia',
]);

const PRODUCE_KEYWORDS = sortKeywordsLongestFirst([
  'sweet potato',
  'sweet potatoes',
  'fingerling potato',
  'russet potato',
  'yukon gold',
  'red potato',
  'baby potato',
  'bell pepper',
  'jalapeño',
  'jalapeno',
  'poblano pepper',
  'serrano pepper',
  'habanero',
  'shishito',
  'banana pepper',
  'cherry tomato',
  'grape tomato',
  'roma tomato',
  'beefsteak tomato',
  'heirloom tomato',
  'green onion',
  'spring onion',
  'scallion',
  'red onion',
  'white onion',
  'yellow onion',
  'vidalia onion',
  'pearl onion',
  'shallot',
  'leek',
  'garlic clove',
  'garlic bulb',
  'fresh garlic',
  'ginger root',
  'fresh ginger',
  'baby carrot',
  'rainbow carrot',
  'celery stalk',
  'celery heart',
  'iceberg lettuce',
  'romaine lettuce',
  'butter lettuce',
  'red leaf lettuce',
  'green leaf lettuce',
  'spring mix',
  'mixed greens',
  'baby spinach',
  'baby kale',
  'microgreens',
  'microgreen',
  'alfalfa sprout',
  'bean sprout',
  'broccoli rabe',
  'broccolini',
  'bok choy',
  'baby bok choy',
  'napa cabbage',
  'red cabbage',
  'green cabbage',
  'savoy cabbage',
  'brussels sprout',
  'brussels sprouts',
  'green bean',
  'green beans',
  'snap pea',
  'snow pea',
  'sugar snap',
  'english pea',
  'garden pea',
  'zucchini',
  'yellow squash',
  'butternut squash',
  'acorn squash',
  'spaghetti squash',
  'delicata squash',
  'kabocha',
  'pumpkin',
  'eggplant',
  'japanese eggplant',
  'cucumber',
  'english cucumber',
  'persian cucumber',
  'pickle cucumber',
  'asparagus',
  'artichoke',
  'artichoke heart',
  'cauliflower',
  'broccoli',
  'romanesco',
  'kohlrabi',
  'celeriac',
  'parsnip',
  'turnip',
  'rutabaga',
  'beet',
  'beets',
  'radish',
  'radishes',
  'daikon',
  'jicama',
  'fennel bulb',
  'fennel',
  'corn on the cob',
  'sweet corn',
  'corn cob',
  'okra',
  'tomatillo',
  'poblano',
  'anaheim pepper',
  'cilantro',
  'fresh cilantro',
  'flat leaf parsley',
  'italian parsley',
  'curly parsley',
  'fresh parsley',
  'fresh basil',
  'thai basil',
  'fresh mint',
  'fresh dill',
  'fresh rosemary',
  'fresh thyme',
  'fresh sage',
  'fresh oregano',
  'chive',
  'chives',
  'tarragon',
  'marjoram',
  'lemongrass',
  'bay leaf',
  'watercress',
  'arugula',
  'rocket',
  'endive',
  'radicchio',
  'frisée',
  'frisee',
  'escarole',
  'chard',
  'swiss chard',
  'collard green',
  'collard greens',
  'mustard green',
  'mustard greens',
  'dandelion green',
  'tatsoi',
  'mizuna',
  'spinach',
  'kale',
  'lettuce',
  'cabbage',
  'mushroom',
  'mushrooms',
  'shiitake',
  'portobello',
  'cremini',
  'oyster mushroom',
  'enoki',
  'maitake',
  'chanterelle',
  'morel',
  'truffle',
  'apple',
  'apples',
  'granny smith',
  'honeycrisp',
  'gala apple',
  'fuji apple',
  'orange',
  'oranges',
  'navel orange',
  'blood orange',
  'cara cara',
  'mandarin',
  'clementine',
  'tangerine',
  'satsuma',
  'lemon',
  'lemons',
  'meyer lemon',
  'lime',
  'limes',
  'key lime',
  'grapefruit',
  'pomelo',
  'ugli fruit',
  'banana',
  'bananas',
  'plantain',
  'strawberry',
  'strawberries',
  'blueberry',
  'blueberries',
  'raspberry',
  'raspberries',
  'blackberry',
  'blackberries',
  'boysenberry',
  'mulberry',
  'currant',
  'gooseberry',
  'elderberry',
  'cranberry',
  'cranberries',
  'cherry',
  'cherries',
  'grape',
  'grapes',
  'watermelon',
  'cantaloupe',
  'honeydew',
  'galia melon',
  'peach',
  'peaches',
  'nectarine',
  'nectarines',
  'apricot',
  'apricots',
  'plum',
  'plums',
  'prune plum',
  'pear',
  'pears',
  'asian pear',
  'mango',
  'mangoes',
  'papaya',
  'guava',
  'passion fruit',
  'dragon fruit',
  'pitaya',
  'kiwi',
  'kiwifruit',
  'pineapple',
  'coconut',
  'young coconut',
  'fig',
  'figs',
  'persimmon',
  'pomegranate',
  'jackfruit',
  'durian',
  'lychee',
  'rambutan',
  'star fruit',
  'carambola',
  'avocado',
  'avocados',
  'tomato',
  'tomatoes',
  'carrot',
  'carrots',
  'celery',
  'potato',
  'potatoes',
  'onion',
  'onions',
  'garlic',
  'ginger',
  'pepper',
  'peppers',
  'jalapeno',
  'squash',
  'melon',
  'berry',
  'berries',
  'fruit',
  'vegetable',
  'veggie',
  'greens',
  'sprout',
  'sprouts',
  'salad',
]);

function categorizeFood(name: string): FoodKind {
  const n = name.toLowerCase();
  for (const kw of PANTRY_KEYWORDS) {
    if (matchesKeyword(n, kw)) return 'grain';
  }
  for (const kw of DAIRY_KEYWORDS) {
    if (matchesKeyword(n, kw)) return 'dairy';
  }
  for (const kw of PROTEIN_KEYWORDS) {
    if (matchesKeyword(n, kw)) return 'meatFish';
  }
  for (const kw of PRODUCE_KEYWORDS) {
    if (matchesKeyword(n, kw)) return 'produce';
  }
  return 'other';
}

export function groceryCategoryFromFood(foodName: string): string {
  const kind = categorizeFood(foodName);
  const map: Record<string, string> = {
    grain: 'Pantry',
    meatFish: 'Meat & Fish',
    produce: 'Produce',
    dairy: 'Dairy',
    other: 'Other',
  };
  return map[kind] ?? 'Other';
}

const GROCERY_JSON_CATEGORY_ORDER = ['Produce', 'Meat & Fish', 'Dairy', 'Pantry', 'Other'] as const;

/** Space-enclosed em dash between food name and amount (must match formatted lines). */
export const GROCERY_LIST_NAME_AMOUNT_SEP = ' \u2014 ';

export type GroceryListJsonEntry = { category: string; item: string; checked: boolean };

/** Case-insensitive merge key; trims and collapses internal whitespace. */
function normalizeIngredientKey(raw: string): string {
  return String(raw ?? '')
    .trim()
    .replace(/\s+/g, ' ')
    .toLowerCase();
}

/**
 * Remove a leading fraction or decimal from serving_size text; remainder is the unit phrase.
 * E.g. "1 medium" → "medium", "1/2 cup dry" → "cup dry".
 */
function stripLeadingNumericPrefixFromServingSize(s: string): string {
  const t = s.trim();
  if (!t) return '';
  const frac = t.match(/^(\d+\/\d+)\s+(.+)$/);
  if (frac) return frac[2].trim();
  const dec = t.match(/^(\d+(?:\.\d+)?)\s+(.+)$/);
  if (dec) return dec[2].trim();
  return t;
}

const PURE_NUMBER_TOKEN = /^\d+(?:\.\d+)?$|^\d+\/\d+$/;

/**
 * Unit for grocery display: strip leading number from serving_size, then drop a trailing parenthetical
 * (e.g. "6 oz (170g)" → "oz"). Returns "" if nothing usable remains.
 */
function extractGroceryUnitFromServingSize(servingSize: string | null | undefined): string {
  const raw = String(servingSize ?? '').trim();
  if (!raw) return '';
  const rest = stripLeadingNumericPrefixFromServingSize(raw);
  if (!rest || PURE_NUMBER_TOKEN.test(rest)) return '';
  let u = rest.replace(/\s*\([^)]*\)\s*$/g, '').trim();
  if (!u || PURE_NUMBER_TOKEN.test(u)) return '';
  return u;
}

function normalizeUnitKey(unit: string): string {
  return unit.trim().toLowerCase().replace(/\s+/g, ' ');
}

/**
 * Load every food line for a meal plan: all PlannedMeals rows for that plan (one template day:
 * breakfast/lunch/dinner/etc.) joined to FoodItems — one row per ingredient per meal slot.
 * Each row gets `quantity` from `FoodItems.quantity` (if present) and/or the leading amount in
 * `serving_size` (e.g. "2 links"). `generateGroceryListFromPlan` multiplies by scheduled days per week.
 * Legacy plans use MealPlanItems (flat list); same weekly multiplier applies.
 */
export async function fetchMealPlanFoodRowsForGrocery(
  db: any,
  planId: number,
): Promise<GroceryPlanFoodRow[]> {
  type DbRow = {
    meal_type: string;
    food_name: string;
    serving_size?: string | null;
    quantity?: number | null;
  };

  let rows: DbRow[] = [];
  try {
    rows = (await db.getAllAsync(
      `SELECT pm.meal_type, fi.food_name, fi.serving_size, fi.quantity AS quantity
     FROM PlannedMeals pm
     JOIN FoodItems fi ON fi.meal_id = pm.meal_id
     WHERE pm.meal_plan_id = ?
     ORDER BY pm.meal_order, pm.meal_id, fi.food_id`,
      [planId],
    )) as DbRow[];
  } catch {
    rows = (await db
      .getAllAsync(
        `SELECT pm.meal_type, fi.food_name, fi.serving_size
     FROM PlannedMeals pm
     JOIN FoodItems fi ON fi.meal_id = pm.meal_id
     WHERE pm.meal_plan_id = ?
     ORDER BY pm.meal_order, pm.meal_id, fi.food_id`,
        [planId],
      )
      .catch(() => [])) as DbRow[];
  }

  if (rows.length === 0) {
    const fallback = (await db
      .getAllAsync(
        'SELECT meal_type, food_name FROM MealPlanItems WHERE meal_plan_id = ? ORDER BY sort_order, item_id',
        [planId],
      )
      .catch(() => [])) as { meal_type: string | null; food_name: string }[];
    rows = fallback.map((r) => ({
      meal_type: r.meal_type || 'Other',
      food_name: r.food_name,
      serving_size: null,
    }));
  }

  return rows.map((r) => ({
    food_name: r.food_name,
    serving_size: r.serving_size ?? null,
    quantity: effectiveGroceryPerMealQuantity(r.serving_size ?? null, r.quantity),
  }));
}

function normalizeGroceryJsonCategory(raw: string): string {
  const c = (raw || 'Other').trim();
  if (c === 'Protein') return 'Meat & Fish';
  if (
    c === 'Produce' ||
    c === 'Meat & Fish' ||
    c === 'Dairy' ||
    c === 'Pantry' ||
    c === 'Other'
  ) {
    return c;
  }
  return 'Other';
}

/**
 * Parse a grocery list line for merging (same format as `buildGroceryListJsonFromPlanFoodRows` output).
 */
export function parseGroceryListItemForAggregation(itemText: string): {
  name: string;
  nameKey: string;
  quantity: number | null;
  unitRaw: string;
  unitNorm: string;
} {
  const t = itemText.trim();
  const sep = GROCERY_LIST_NAME_AMOUNT_SEP;
  const idx = t.indexOf(sep);
  if (idx === -1) {
    const name = t.replace(/\s+/g, ' ');
    return {
      name,
      nameKey: normalizeIngredientKey(name),
      quantity: null,
      unitRaw: '',
      unitNorm: '',
    };
  }
  const name = t.slice(0, idx).trim().replace(/\s+/g, ' ');
  const tail = t.slice(idx + sep.length).trim();

  const fracUnit = tail.match(/^(\d+)\s*\/\s*(\d+)\s+(.+)$/);
  if (fracUnit) {
    const num = parseInt(fracUnit[1], 10);
    const den = parseInt(fracUnit[2], 10);
    const q = den !== 0 && Number.isFinite(num) ? num / den : null;
    const u = fracUnit[3].trim();
    return {
      name,
      nameKey: normalizeIngredientKey(name),
      quantity: q,
      unitRaw: u,
      unitNorm: normalizeUnitKey(u),
    };
  }

  const decUnit = tail.match(/^(\d+(?:\.\d+)?)\s+(.+)$/);
  if (decUnit) {
    const q = parseFloat(decUnit[1]);
    const u = decUnit[2].trim();
    return {
      name,
      nameKey: normalizeIngredientKey(name),
      quantity: Number.isFinite(q) ? q : null,
      unitRaw: u,
      unitNorm: normalizeUnitKey(u),
    };
  }

  const decOnly = tail.match(/^(\d+(?:\.\d+)?)$/);
  if (decOnly) {
    const q = parseFloat(decOnly[1]);
    return {
      name,
      nameKey: normalizeIngredientKey(name),
      quantity: Number.isFinite(q) ? q : null,
      unitRaw: '',
      unitNorm: '',
    };
  }

  const fracOnly = tail.match(/^(\d+)\s*\/\s*(\d+)$/);
  if (fracOnly) {
    const num = parseInt(fracOnly[1], 10);
    const den = parseInt(fracOnly[2], 10);
    const q = den !== 0 && Number.isFinite(num) ? num / den : null;
    return {
      name,
      nameKey: normalizeIngredientKey(name),
      quantity: q,
      unitRaw: '',
      unitNorm: '',
    };
  }

  const nameOnly = t.replace(/\s+/g, ' ');
  return {
    name: nameOnly,
    nameKey: normalizeIngredientKey(nameOnly),
    quantity: null,
    unitRaw: '',
    unitNorm: '',
  };
}

/**
 * Merge grocery JSON lines that share the same food name (case-insensitive) and unit; sum quantities.
 * Lines with different units stay separate. Name-only lines count as quantity 1 each when merged.
 */
export function aggregateGroceryListJsonEntriesByNameAndUnit(
  entries: { category: string; item: string; checked?: boolean }[],
): GroceryListJsonEntry[] {
  type Agg = {
    category: string;
    displayName: string;
    totalQty: number;
    unitRaw: string;
    unitNorm: string;
  };
  const map = new Map<string, Agg>();

  for (const e of entries) {
    const category = normalizeGroceryJsonCategory(e.category);
    const p = parseGroceryListItemForAggregation(e.item);
    const key = `${category}\0${p.nameKey}\0${p.unitNorm}`;
    const qtyAdd =
      p.quantity != null && Number.isFinite(p.quantity) && p.quantity > 0
        ? p.quantity
        : 1;

    const ex = map.get(key);
    if (!ex) {
      map.set(key, {
        category,
        displayName: p.name,
        totalQty: qtyAdd,
        unitRaw: p.unitRaw,
        unitNorm: p.unitNorm,
      });
      continue;
    }
    ex.totalQty += qtyAdd;
  }

  const byCategory: Record<string, string[]> = {
    Produce: [],
    'Meat & Fish': [],
    Dairy: [],
    Pantry: [],
    Other: [],
  };

  for (const row of map.values()) {
    const rounded = Math.round(row.totalQty * 1000) / 1000;
    if (rounded <= 0) continue;
    const amountStr = Number.isInteger(rounded)
      ? String(Math.round(rounded))
      : String(rounded);
    const sep = GROCERY_LIST_NAME_AMOUNT_SEP;
    const unit = row.unitRaw.trim();
    let item: string;
    if (unit) {
      item = `${row.displayName}${sep}${amountStr} ${unit}`;
    } else if (Math.abs(rounded - 1) > 1e-9) {
      item = `${row.displayName}${sep}${amountStr}`;
    } else {
      item = row.displayName;
    }
    if (byCategory[row.category]) byCategory[row.category].push(item);
    else byCategory['Other'].push(item);
  }

  const list: GroceryListJsonEntry[] = [];
  for (const cat of GROCERY_JSON_CATEGORY_ORDER) {
    for (const item of byCategory[cat]) list.push({ category: cat, item, checked: false });
  }
  return list;
}

/**
 * Build grocery JSON: merge rows by normalized `food_name` + grocery unit (from `serving_size`), sum
 * `(quantity ?? 1) × familySize` per matching row. Same food with different units produces separate lines.
 * Lines are `[food name] — [total] [unit]`, name only when total is 1 and no unit, or `[name] — [total]` when no unit but total ≠ 1.
 */
export function buildGroceryListJsonFromPlanFoodRows(
  rows: GroceryPlanFoodRow[],
  options?: { familySize?: number },
): GroceryListJsonEntry[] {
  const familySize = Math.max(1, Math.min(8, options?.familySize ?? 1));
  const aggregate = new Map<
    string,
    {
      displayName: string;
      category: string;
      totalQty: number;
      unit: string;
    }
  >();

  for (const r of rows) {
    const displayName = String(r.food_name ?? '').trim().replace(/\s+/g, ' ');
    if (!displayName) continue;
    const unitThis = extractGroceryUnitFromServingSize(r.serving_size);
    const unitKeyThis = unitThis ? normalizeUnitKey(unitThis) : '';
    const nameKey = normalizeIngredientKey(displayName);
    const aggregateKey = `${nameKey}\0${unitKeyThis}`;
    const category = groceryCategoryFromFood(displayName);
    const rowQty = rowGroceryQuantity(r) * familySize;
    if (rowQty <= 0) continue;

    const existing = aggregate.get(aggregateKey);
    if (!existing) {
      aggregate.set(aggregateKey, {
        displayName,
        category,
        totalQty: rowQty,
        unit: unitThis,
      });
      continue;
    }

    existing.totalQty += rowQty;
  }

  const byCategory: Record<string, string[]> = {
    Produce: [],
    'Meat & Fish': [],
    Dairy: [],
    Pantry: [],
    Other: [],
  };

  const sep = GROCERY_LIST_NAME_AMOUNT_SEP;
  for (const row of aggregate.values()) {
    const rounded = Math.round(row.totalQty * 1000) / 1000;
    if (rounded <= 0) continue;
    const amountStr = Number.isInteger(rounded)
      ? String(Math.round(rounded))
      : String(rounded);

    const unit = row.unit.trim();
    let item: string;
    if (unit) {
      item = `${row.displayName}${sep}${amountStr} ${unit}`;
    } else if (Math.abs(rounded - 1) > 1e-9) {
      item = `${row.displayName}${sep}${amountStr}`;
    } else {
      item = row.displayName;
    }

    if (byCategory[row.category]) byCategory[row.category].push(item);
    else byCategory['Other'].push(item);
  }
  const list: GroceryListJsonEntry[] = [];
  for (const cat of GROCERY_JSON_CATEGORY_ORDER) {
    for (const item of byCategory[cat]) list.push({ category: cat, item, checked: false });
  }
  return list;
}

/**
 * Grocery JSON from nutrition-tracker rows (e.g. LoggedFoods for a date range).
 * Same rules as `buildGroceryListJsonFromPlanFoodRows`: sum `quantity` per food, unit from `serving_size` after stripping a leading number.
 */
export function buildGroceryListJsonFromUniqueFoodNames(
  rows: GroceryPlanFoodRow[],
  options?: { familySize?: number },
): GroceryListJsonEntry[] {
  return buildGroceryListJsonFromPlanFoodRows(rows, options);
}

export async function generateGroceryListFromPlan(db: any, planId: number): Promise<void> {
  const rawRows = await fetchMealPlanFoodRowsForGrocery(db, planId);
  const weekDays = await countWeeklyPlanDaysForGrocery(db, planId);
  const rows = multiplyPlanFoodRowsForWeek(rawRows, weekDays);
  const list = buildGroceryListJsonFromPlanFoodRows(rows, { familySize: 1 });
  await db.runAsync('UPDATE MealPlans SET grocery_list = ? WHERE meal_plan_id = ?', [
    JSON.stringify(list),
    planId,
  ]);
}

/** Case-insensitive dedupe of food names; keeps first-seen casing. */
export function dedupeFoodNamesCaseInsensitive(names: string[]): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const raw of names) {
    if (raw == null) continue;
    const t = String(raw).trim();
    if (!t) continue;
    const k = t.toLowerCase();
    if (seen.has(k)) continue;
    seen.add(k);
    out.push(t);
  }
  return out;
}

/**
 * Build prep guide text from raw food names (deduped case-insensitively),
 * using the same category buckets as per-plan prep (grain/pantry, meat & fish, produce, dairy, other).
 */
export function buildPrepGuideTextFromUniqueFoodNames(names: string[]): string {
  const uniqueFoods = dedupeFoodNamesCaseInsensitive(names);
  if (uniqueFoods.length === 0) return '';

  const byCategory: Record<string, string[]> = {
    grain: [],
    meatFish: [],
    produce: [],
    dairy: [],
    other: [],
  };
  for (const name of uniqueFoods) {
    const cat = categorizeFood(name);
    byCategory[cat].push(name);
  }
  const steps: string[][] = [];
  if (byCategory.grain.length > 0) {
    steps.push([
      'Pantry, grains & dry goods',
      `Stock and prep shelf-stable items and starches: ${byCategory.grain.join(', ')}. Portion where helpful; store per label.`,
    ]);
  }
  if (byCategory.meatFish.length > 0) {
    steps.push([
      'Cook & portion meat & fish',
      `Prep and cook: ${byCategory.meatFish.join(', ')}. Portion into containers for each meal so you can grab and go.`,
    ]);
  }
  if (byCategory.produce.length > 0) {
    steps.push([
      'Wash and chop produce',
      `Wash, dry, and chop: ${byCategory.produce.join(', ')}. Store in containers or bags. Pre-portion for snacks if needed.`,
    ]);
  }
  if (byCategory.dairy.length > 0) {
    steps.push([
      'Portion dairy',
      `Portion out ${byCategory.dairy.join(', ')} into single-serving containers or add to meal containers as needed.`,
    ]);
  }
  steps.push([
    'Portion snacks and breakfast items',
    'Portion out any grab-and-go snacks. Prep overnight oats, egg muffins, or other breakfast items so mornings are easy.',
  ]);
  steps.push([
    'Assemble and store',
    "Label containers by day or meal (e.g. Mon Lunch, Tue Dinner). Keep in the fridge; freeze any portions you won't use within 3–4 days.",
  ]);
  const lines: string[] = ['Meal prep for the week — follow these steps in order.', ''];
  steps.forEach(([title, body], i) => {
    lines.push(`${i + 1}. ${title}`);
    lines.push(`   ${body}`);
    lines.push('');
  });
  lines.push(
    'Your plan includes: ' + uniqueFoods.slice(0, 12).join(', ') + (uniqueFoods.length > 12 ? '...' : '.'),
  );
  return lines.join('\n').trim();
}

export async function generatePrepGuideFromPlan(db: any, planId: number): Promise<void> {
  let rows: { meal_type: string; food_name: string }[] = await db
    .getAllAsync(
      `SELECT pm.meal_type, fi.food_name
     FROM PlannedMeals pm
     JOIN FoodItems fi ON fi.meal_id = pm.meal_id
     WHERE pm.meal_plan_id = ?
     ORDER BY pm.meal_order, fi.food_id`,
      [planId],
    )
    .catch(() => []);
  if (rows.length === 0) {
    const fallback = (await db
      .getAllAsync(
        'SELECT meal_type, food_name FROM MealPlanItems WHERE meal_plan_id = ? ORDER BY sort_order, item_id',
        [planId],
      )
      .catch(() => [])) as { meal_type: string | null; food_name: string }[];
    rows = fallback.map((r) => ({ meal_type: r.meal_type || 'Meal', food_name: r.food_name }));
  }
  const text = buildPrepGuideTextFromUniqueFoodNames(rows.map((r) => r.food_name));
  await db.runAsync('UPDATE MealPlans SET prep_guide = ? WHERE meal_plan_id = ?', [text || null, planId]);
}
