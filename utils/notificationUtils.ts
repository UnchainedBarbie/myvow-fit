...
/** Show a one-time celebration notification (e.g. weight loss, PR). Fires after 1 second so it appears right after the action. */
export const showCelebrationNotification = async (
  title: string,
  body: string
): Promise<string | null> => {
  try {
    ...
    const id = await Notifications.scheduleNotificationAsync({
      content: { title, body, data: { type: 'celebration' } },
      trigger: {
        seconds: 1,
    ...
    console.error('Error showing celebration notification:', error);
    return null;
  }
};
