...
export const addRestSecondsColumn = async (db: SQLiteDatabase) => {
    const tables = ['Exercises', 'Logged_Exercises'];
    try {
        for (const table of tables) {
            const columns = await db.getAllAsync<any>(`PRAGMA table_info(${table});`);
            const columnExists = columns.some((col: any) => col.name === 'rest_seconds');

            if (!columnExists) {
                await db.runAsync(`ALTER TABLE ${table} ADD COLUMN rest_seconds INTEGER;`);
                console.log(`'rest_seconds' column added to '${table}' table.`);
            }
        }
    } catch (error) {
        console.error('Error adding rest_seconds column:', error);
    }
};